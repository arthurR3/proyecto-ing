...
export function register(config) {
 ...
 /*  export function register(config) {
    ...
    if ((process.env.NODE_ENV === 'production' || isLocalhost) && 'serviceWorker' in navigator) {
      ...
      window.addEventListener('load', () => {
        // Asegura que en local `swUrl` apunte a la ubicación del service worker local
        const swUrl = isLocalhost ? '/cw.js' : `${process.env.PUBLIC_URL}/service-worker.js`;
  
        if (isLocalhost) {
          checkValidServiceWorker(swUrl, config);
  
          navigator.serviceWorker.ready.then(() => {
            console.log('Service Worker registrado en modo desarrollo en localhost.');
          });
        } else {
          registerValidSW(swUrl, config);
        }
